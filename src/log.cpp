...
void __cdecl ConsoleLog(const char *format, ...)
{
	...
	*p = '\0';
	...
	std::clog << FormatTitle << "[disguise] ";
	std::clog << FormatText << buf;
}

void __cdecl ConsoleDisguiseOut(const char *format, ...)
{
	char    buf[4096], * p = buf;
	va_list args;
	int     n;
	...
	*p++ = '\n';
	*p = '\0';
	EndOfLineEscapeTag FormatTitle{ CONSOLE_COLOR_YELLOW, ANSI_TEXT_COLOR_RESET };
	EndOfLineEscapeTag FormatText{ CONSOLE_COLOR_BLUE, ANSI_TEXT_COLOR_RESET };
	std::clog << FormatTitle << "[hide] ";
	std::clog << FormatText << buf;
}
...
void __cdecl ConsoleError(const char *format, ...)
{
	...
	*p++ = '\n';
	*p = '\0';
	EndOfLineEscapeTag FormatTitle1{ ANSI_TEXT_COLOR_YELLOW_BKGRND, CONSOLE_COLOR_RED };
	EndOfLineEscapeTag FormatTitle2{ CONSOLE_COLOR_RED, ANSI_TEXT_COLOR_RESET };
	EndOfLineEscapeTag FormatText{ CONSOLE_COLOR_RED, ANSI_TEXT_COLOR_RESET };
	std::clog << FormatTitle1 << "";
	std::clog << FormatTitle2 << "[error] ";
	std::clog << FormatText << buf;
}
//==============================================================================
// SystemDebugOutput
