...
#define APP_NAME_SMALL		"Plape"
#define APP_NAME_BIG		"Plape - Process Launcher with Automatic Privileges Escalation"
#define EXECUTABLE_NAME		"plauncher.exe"
#define APP_VERSION			"v2.0.1"
#define APP_COPYRIGHT		"cybercastor <cybercastor@icloud.com>"
...
{
	std::cout << APP_NAME_BIG << std::endl;
	std::cout << APP_VERSION << " - " << APP_COPYRIGHT << std::endl << std::endl;
}
...
void CmdLineUtil::printSyntax()
{
	...
	std::cout << " [-s --server-list] <options> <filters>\n\tList available license servers on the network." << std::endl;
	std::cout << " [-s --server-list --serial <id> | --firm <id> | --product <id>]" << std::endl;
	std::cout << "\tApply filter to the server list. If you specify a firm id, only server with a FSB that\n\thas that firm id will be returned." << std::endl;
	std::cout << " [-s --server-list --local]" << std::endl;
	std::cout << "\tServer list includes local server." << std::endl;
	std::cout << " [-s --server-list --empty]" << std::endl;
	std::cout << "\tServer list includes server that have no FSB or any dongles." << std::endl << std::endl;
}
