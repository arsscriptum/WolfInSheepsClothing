...
void decomposePath(const TCHAR *filePath, TCHAR *fileDir, TCHAR *fileName, TCHAR *fileExt)
{
	...
	if (fileDir)
		__SPRINTF(fileDir, MAX_PATH, _T("%.*s"),int( startOfName - filePath), filePath);

	if (fileName)
		__SPRINTF(fileName, MAX_PATH, _T("%.*s"), int(startOfExt - startOfName), startOfName);

	if (fileExt)
		__SPRINTF(fileExt, MAX_PATH, _T("%s"), startOfExt);
